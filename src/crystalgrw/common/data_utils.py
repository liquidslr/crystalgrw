...
def lattice_params_from_matrix(matrix):
    ...
    angles = angles[:, [2,0,1], [0,1,2]] * 180 / np.pi
...
def lattice_params_to_matrix_torch(lengths, angles):
    ...
    val = (-coses[:, 2] * coses[:, 1] + coses[:, 0]) / (sins[:, 2] * sins[:, 1])
    # Sometimes rounding errors result in values slightly > 1.
    ...
    vector_c = torch.stack([
        lengths[:, 2] * coses[:, 1],
        lengths[:, 2] * sins[:, 1] * torch.cos(gamma_star) ,
...
def radius_graph_pbc(cart_coords, lattice, num_atoms,
                     radius, max_num_neighbors_threshold, device,
                     topk_per_pair=None, pbc=[True, True, True]):
    ...
    cell_vol = torch.sum(lattice[:, 0] * cross_a2a3, dim=-1, keepdim=True)

    if pbc[0]:
        inv_min_dist_a1 = torch.norm(cross_a2a3 / cell_vol, p=2, dim=-1)
        rep_a1 = torch.ceil(radius * inv_min_dist_a1)
    else:
        rep_a1 = lattice.new_zeros(1)
    ...
    # cells_per_dim = [
    #     torch.arange(-rep, rep + 1, device=device, dtype=DTYPE)
    #     for rep in max_rep
    # ]
...
#         lengths = graph_arrays[3]
#         angles = graph_arrays[4]
#
...
def add_scaled_lattice_prop(data_list, lattice_scale_method):
    for data_dict in data_list:
        ...
        if lattice_scale_method == "scale_length":
            lengths = lengths / float(num_atoms)**(1/3)
        data_dict["scaled_lattice"] = np.concatenate([lengths, angles])
...
def batch_accuracy_precision_recall(
    ...
    if (pred_edge_probs is None and edge_overlap_mask is None and
            num_bonds is None):
        return 0., 0., 0.
    pred_edges = pred_edge_probs.max(dim=1)[1].to(DTYPE)
    target_edges = edge_overlap_mask.to(DTYPE)
...
def squeeze_sample(samples):
    for k in samples:
        try:
            samples[k] = samples[k].squeeze(0)
        except Exception as e:
            pass
    return samples


def get_ase_atoms(samples):
    samples = squeeze_sample(samples)
    atoms = []
    ...
    for i, m in enumerate(tqdm(range(samples["num_atoms"].shape[1]), desc=f"Save XYZ")):
        for n in range(samples["num_atoms"].shape[0]):
            num_atoms = samples["num_atoms"][n, m].item()
            angles = samples["angles"][n, m].numpy()
            lengths = samples["lengths"][n, m].numpy()
            atom_types = samples["atom_types"][n, j:j + num_atoms].numpy()
            frac_coords = samples["frac_coords"][n, j:j + num_atoms].numpy()

            atom = Atoms(
                scaled_positions=frac_coords,
                numbers=atom_types,
                cell=np.concatenate([lengths, angles], axis=0),
                pbc=[True, True, True],
            )
            atoms.append(atom)
...
def get_ase_traj_atoms(samples):
    samples = squeeze_sample(samples)
    T = samples["traj_lattices"].shape[1]
    traj_atoms = []
    ...
    for i, m in enumerate(tqdm(range(samples["num_atoms"].shape[1]), desc=f"Save XYZ")):
        for n in range(samples["num_atoms"].shape[0]):
            num_atoms = samples["num_atoms"][n, m].item()
            lattices = samples["traj_lattices"][n, m].numpy()
            atom_types = samples["traj_atom_types"][n, j:j + num_atoms].numpy()
            frac_coords = samples["traj_frac_coords"][n, j:j + num_atoms].numpy()

            for t in range(T):
                atom = Atoms(
                    scaled_positions=frac_coords[:, t],
                    numbers=atom_types[:, t],
                    cell=lattices[t],
                    pbc=[True, True, True],
                )
                atom.info["traj_info"] = f"struct{i}_eval{n}_t{(T - t - 1):04d}"
                traj_atoms.append(atom)
