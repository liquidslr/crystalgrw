...
from .dataset import TensorCrystDataset
from .datamodule import CrystDataModule, worker_init_fn
...
def get_model_path(eval_model_name):
    import models 
    model_path = (
        Path(models.__file__).parent / "prop_models" / eval_model_name)
    return model_path
...
def load_model(model, model_path, cfg, load_data=False, testing=True, prop_model=False):
    # cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/"hparams.yaml")), resolve=True))
    hparams = cfg.model
    ...
    if not prop_model:
        ...
        model.init(training=(not testing), load_data=load_data)
    else:        
    ...
    if load_data:
        datamodule = CrystDataModule(**{k: v for k, v in cfg.data.datamodule.items() if k!="_target_"},
                                     dataset=cfg.data.datamodule._target_, training=(not testing))
        datamodule.setup(not testing)
...
def prop_model_eval(eval_model_name, crystal_array_list):
    ...
    cfg = OmegaConf.create(OmegaConf.to_container(OmegaConf.load(str(model_path/"hparams.yaml")), resolve=True))
    model, _ = load_model(CrystGNN_Supervise, model_path, cfg, prop_model=True)
    # or
