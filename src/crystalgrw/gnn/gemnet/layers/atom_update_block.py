...
class OutputBlock(AtomUpdateBlock):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.regress_atoms:
            self.scale_rbf_A = ScalingFactor(
                    scale_file=scale_file, name=name + "_had"
                )
            self.seq_atom = self.layers
            self.out_atom = Dense(
                emb_size_atom, MAX_ATOMIC_NUM, bias=False, activation=None
            )
            self.dense_rbf_A = Dense(
                    emb_size_rbf, emb_size_edge, activation=None, bias=False
                )
        ...
        if self.regress_lattices:
            self.scale_rbf_L = ScalingFactor(
                scale_file=scale_file, name=name + "_had"
            )
            self.seq_lattices = self.get_mlp(
                emb_size_edge, emb_size_edge, nHidden, activation
            )
            self.out_lattices = Dense(
                emb_size_edge, 9, bias=False, activation=None
            )
            self.dense_rbf_L = Dense(
                emb_size_rbf, emb_size_edge, activation=None, bias=False
            )
    ...
    def forward(self, h, m, rbf, id_j):
        ...
        # -------------------------------------- Atom Prediction -------------------------------------- #
        if self.regress_atoms:
            rbf_emb_A = self.dense_rbf_A(rbf)  # (nEdges, emb_size_edge)
            x_A = m * rbf_emb_A

            x_A = scatter(x_A, id_j, dim=0, dim_size=nAtoms, reduce="sum")
            # (nAtoms, emb_size_edge)
            #x_A = self.scale_sum_A(m, x_A)

            for layer in self.seq_atom:
                x_A = layer(x_A)  # (nAtoms, emb_size_atom)

            x_A = self.out_atom(x_A)  # (nAtoms, num_targets)
        else:
            x_A = 0
        ...
        # --------------------------------------- Lattice Prediction -------------------------------------- #
        if self.regress_lattices:
            rbf_emb_L = self.dense_rbf_L(rbf)  # (nEdges, emb_size_edge)
            x_L = m * rbf_emb_L

            x_L = scatter(x_L, id_j, dim=0, dim_size=nAtoms, reduce="sum")
            # (nAtoms, emb_size_edge)
            #x_A = self.scale_sum_A(m, x_A)

            for layer in self.seq_lattices:
                x_L = layer(x_L)  # (nAtoms, emb_size_atom)

            x_L = self.out_lattices(x_L)  # (nAtoms, num_targets)
        else:
            x_L = 0
